...
type mutableReloadFailConfig struct {
	Structure
	Settings func() map[string]string `cfggo:"settings"`
	Port     func() int               `cfggo:"port" default:"8080"`
}

// When reload fails (e.g. invalid JSON or validation failure on a subsequent key),
// the rollback must not leave mutated maps/slices in configData if they were modified
// prior to the failure.
func TestReloadRollbackPreservesMutableData(t *testing.T) {
	oldArgs := os.Args
	t.Cleanup(func() { os.Args = oldArgs })
	os.Args = []string{"test"}

	initialJSON := []byte(`{"settings":{"env":"prod","region":"us-east"},"port":8080}`)
	handler := &memHandler{data: initialJSON}

	cfg := &mutableReloadFailConfig{}
	if err := cfg.Init(cfg, WithConfigHandler(handler), WithoutFlags(), WithoutEnv()); err != nil {
		t.Fatalf("Init: %v", err)
	}

	if got := cfg.Settings()["env"]; got != "prod" {
		t.Fatalf("Settings[env] = %q, want prod", got)
	}

	// Hand over bad data that alters settings and fails on invalid JSON or bad type
	badJSON := []byte(`{"settings":{"env":"staging","region":"eu-west"},"port":"not-an-int"}`)
	handler.data = badJSON

	err := cfg.ReloadConfig()
	if err == nil {
		t.Fatal("ReloadConfig expected error, got nil")
	}

	// Verify that rollback preserved the original map content
	settings := cfg.Settings()
	if got := settings["env"]; got != "prod" {
		t.Fatalf("Settings[env] after failed reload = %q, want prod", got)
	}
	if got := settings["region"]; got != "us-east" {
		t.Fatalf("Settings[region] after failed reload = %q, want us-east", got)
	}
}
