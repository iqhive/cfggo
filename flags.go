...
func (c *Structure) newFlag(configVarName string, defaultValue interface{}, configDescription string) {
	...
	// The flag name is prefixed when this Structure is part of a cfggo.Group,
	// but the underlying config key (and the setter) stays unprefixed so
	// accessors continue to work exactly as in standalone use.
...
func (c *Structure) parseFlags() error {
	...
	if c.ignoreUnknownVars {
		...
		args = filterKnownFlags(c.flagSet, args, c.log())
...
// firstUnknownFlag scans args for the first flag that is not registered on fs.
...
func (c *Structure) filterKnownFlags(args []string) []string {
	return filterKnownFlags(c.flagSet, args, c.log())
}

// filterKnownFlags is the package-level implementation used by Structure and
// cfggo.Group.
func filterKnownFlags(fs *flag.FlagSet, args []string, log cfglogger.Logger) []string {
	...
	for i := 0; i < len(args); i++ {
		...
		if f == nil {
			if log != nil {
				log.Debug("cfggo: ignoring unrecognized flag (not defined on this config)", "flag", arg)
			}
...
// normalizeBoolFlagArgs is the package-level implementation used by Structure
// and cfggo.Group.
