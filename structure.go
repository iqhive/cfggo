...
type Structure struct {
	...
	// flagNamePrefix is prepended to the flag name (but not the config key) so
	// multiple Structures can register on a single shared FlagSet without
	// colliding. It is set by cfggo.Group for namespaced members and is empty
	// (zero-cost) for all standalone users.
